...
const DRDialog: FC<PropsWithChildren<Readonly<{
  open: boolean,
  imageUrl: string,
  onClose: () => void
  ...
  return (
    <BaseScreen title='Quests'>
      <Box component='ul' sx={{ listStyle: 'none', p: 0, m: 0 }}>
        {droids?.filter((d) => d.activated).map((d, idx) => <Box component='li' sx={{
          mb: 2,
        }}>
          <>
            {idx === 0 && <Typography variant='body2' color="primary" gutterBottom>Current</Typography>}
            <QuestItem
              variant={currentQuestId === d.order
                ? 'current'
                : !droids[idx + 1]?.activated
                  ? 'active'
                  : 'done'
              }
              description={d.hint}
      ...
      <DRDialog imageUrl={current ? `/droids/${current.name.toLowerCase()}.jpg` : ''} open={!!current} onClose={() => setCurrent(null)}>
        <Stack direction='column' gap={2}>
          ...
          <Button sx={{ justifySelf: 'center' }} onClick={() => {
            console.log(current);
